...
The term `Sleeping Beauty' has been used to describe an article that is not well cited in the early years after its publication but experiences a sharp increase in the rate at which it is subsequently cited~\citep{Raan2004}. One implication is that the new idea(s) presented in such an article is `ahead of its time' and recognition is consequently delayed. Other causes for delayed recognition have been postulated that include information overload from the large amount of information available, modest communication skills of authors, insufficient promotion of ideas, conflict with existing theory and experimental data, the author's position in the social hierarchy of science, multiple discovery, the extent to which scientific institutions are established and organized, and the conservative nature of the establishment~\citep{Barber1961,Merton1963,Cole1970,Garfield1970a,Garfield1980a}. The phenomenon and the term used to describe it, Sleeping Beauty, have been further studied and debated, and some degree of agreement exists that a fraction of the scientific literature exhibits citation kinetics that suggest delayed but eventual recognition of a new idea~\citep{Glanzel2003,Glanzel2004,redner_2005,Braun_2010,Li2014,Ke2015,Li2016,Song2018,sugimoto_mostafa_2018}. 

For identifiying Sleeping Beauties, van Raan, and colleagues propose a classification~\citep{Raan2004}, with subsequent refinements~\citep{Raan2019}, that consider length of sleep, depth of sleep, and awake intensity as parameters. [What did van Raan conclude] \cite{Ke2015} presented a parameter free approach to reduce arbitrariness in parameters selection, and also examined a much larger dataset of 22 million publications to identify Sleeping Beauties. [Need to read what Song and Ye did- also the size of data looked at by van Raan, Redner Ye and others].

However, the work described above has focused on single publications and new ideas also result from the combination of two previously independent ones~\citep{MarshakovaShaikevich1973,Small1973}. Novelty in co-citation from different perspectives has been the subject of several studies ~\citep{Uzzi2013,Boyack2014,Wang2017,Bradley2020} that followed the defining studies of Marshakova-Shaikevich and Small in 1973. he strength of co-citation, measured by its frequency, can be interpreted as the extent to which such new ideas are recognized within scientific communities and the rate at which this frequency enables identfication of Sleeping Beauty analogs, that we term delayed co-citations.
...
In this study, we refine the criteria of Devarakonda et al. towards detecting delayed co-citations and examine a much larger dataset, approximately 940 million pairs of articles. We also ask whether delayed co-citations are derived from Sleeping Beauty publications. Using stringent criteria for selection, we identify a small number of co-cited pairs that would qualify for delayed recognition. Finally, we compare, modifications of a parameter free approach to the more traditional parameterized approaches in the literature.
...
We then calculated the kinetics with which co-citation frequencies accumulated by calculating the annual number of co-citations for each pair from the first possible year of co-citation (the more recent of the publication years of a pair) through 2018. All calculations were done using a combination of SQL, Cypher, and Python scripts. 

The workflow consists of the following steps: (1) We copied 95,524,693 publications from a Postgres DB into a Neo4j DB by exporting the graph to CSVs, importing CSVs into a new database using a Neo4j Bulk Import script, activating the new database and creating indexes (2) We wrote Cypher query to calculate the co-cited pairs for each year in Neo4j and concatenated results to get the 940 million pairs (3) We then calculated the frequency of all pairs across all Scopus in Neo4j by splitting input pairs into small size (1000 pairs) batches and processing them in parallel using GNU Parallel utility. We tuned the batch size and a degree of parallelization (that is, CPU cores used). (4) We loaded frequencies into Postgres, selected all pairs with frequency >= 100 and calculated kinetics for them in Neo4j. Kinetics is the number of co-citations that each pair received in each year. We stored the results in Postgres. (5) Finally, in order to generate complete kinetics for all pairs for further analysis, we calculated the first possible year of co-citation, which is the most recent publication year of a pair, and filled in all missing years between the first possible year and the last co-cited year with frequency = 0 in Python.

...
We also examined co-citation frequencies between 20 and 100 and observed 5,928,815 such pairs. When removed pairs (i) have less than 10 years kinetics in general, which means the different between its publication year and peak year is less than 10 years (ii) the Beauty Coefficient is less or equal to 0, which means such paper's citations is growing linearly with time or its citation trajectory is a convex function of time, and based on our criteria such papers can't be identified as Sleeping Beauties (iii) didn't have one peak of frequency as 20, which is also required as before, then the number reduced to 13,057 pairs. Of these 12,920 had only a single peak of 20 or greater that may be analogous to the `flash in the pan` citation type. [need to cite]. Again, given our focus on frequently co-cited pairs, we do not study these further but they could be the subject of future work. 
